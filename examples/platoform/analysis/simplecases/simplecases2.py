...
s1mint, s1maxt = 55, 110
df = df[(df.time >= s1mint) & (df.time <= s1maxt)]
...
axs[0].axvline(x = 60.83, color = 'k', ls='--', lw=0.8) #PFrequest sent
axs[0].axvline(x = 62.33, color = 'k', ls='--', lw=0.8) #LaneChange completed
axs[0].axvline(x = 100.93, color = 'k', ls='--', lw=0.8) #PFCompleteAck
axs[0].axvline(x = 105.94, color = 'k', ls='--', lw=0.8) #ManeuverEnd

#axs[1].axvline(x = 50, color = 'k', ls='--', lw=0.8) #startPFapp
axs[1].axvline(x = 60.83,  color = 'k', ls='--', lw=0.8) #PFrequest sent
axs[1].axvline(x = 62.33,  color = 'k', ls='--', lw=0.8) #LaneChange completed
axs[1].axvline(x = 100.93, color = 'k', ls='--', lw=0.8) #PFCompleteAck
axs[1].axvline(x = 105.94, color = 'k', ls='--', lw=0.8) #ManeuverEnd
...
plt.subplots_adjust(left=0.11, bottom=0.12, right=0.98, top=0.87, wspace=0.1)
plt.savefig("simplecase2.pdf", format='pdf')
