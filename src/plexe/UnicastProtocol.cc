...
void UnicastProtocol::sendMessageDown(int destination, cPacket* msg, int encapsulatedId, int priority, SimTime timestamp, Channel channel, short kind)
{
    ...
    unicast->setChannel(static_cast<int>(channel));
    ...
    wsm->setChannelNumber(static_cast<int>(channel));
