...
void JoinManeuverApp::initialize(int stage) {
	...
	if (stage == 1) {
		...
		std::string strId = traci->getExternalId();
		std::stringstream ss;
...
void JoinManeuverApp::prepareManeuverCars(int platoonLane) {
    ...
    switch (myId) {
        ...
        case 0: {
            //this is the leader
            traci->commandSetCruiseControlDesiredSpeed(traci->getExternalId(), 100.0 / 3.6);
            traci->commandSetActiveController(traci->getExternalId(), Plexe::ACC);
            traci->commandSetFixedLane(traci->getExternalId(), platoonLane);
            role = LEADER;
        ...
        case 3: {
            ...
            traci->commandSetCruiseControlDesiredSpeed(traci->getExternalId(), 130.0 / 3.6);
            traci->commandSetActiveController(traci->getExternalId(), Plexe::CACC);
            traci->commandSetFixedLane(traci->getExternalId(), platoonLane);
            role = FOLLOWER;
        ...
        case 4: {
            ...
            traci->commandSetCruiseControlDesiredSpeed(traci->getExternalId(), 100/3.6);
		...
		case FSM_Exit(JS_WAIT_REPLY): {
			...
			if (maneuver && maneuver->getPlatoonId() == vehicleData.platoonId) {
			    ...
			    if (maneuver->getMessageType() == LM_MOVE_IN_POSITION) {
			        ...
			        int currentLane = traci->commandGetLaneIndex(traci->getExternalId());
			        if (currentLane != vehicleData.joinLane) {
			            traci->commandSetFixedLane(traci->getExternalId(), vehicleData.joinLane);
			        }

			        //activate faked CACC. this way we can approach the front car using data obtained through GPS
			        traci->commandSetCACCConstantSpacing(traci->getExternalId(), 15);
			        //we have no data so far, so for the moment just initialize with some fake data
			        traci->commandSetControllerFakeData(traci->getExternalId(), 15, vehicleData.speed, 0, vehicleData.speed, 0);
			        //set a CC speed higher than the platoon speed to approach it
			        traci->commandSetCruiseControlDesiredSpeed(traci->getExternalId(), vehicleData.speed + 30/3.6);
			        traci->commandSetActiveController(traci->getExternalId(), Plexe::FAKED_CACC);
		...
		case FSM_Exit(JS_MOVE_IN_POSITION): {
			...
			if (beacon) {
				if (beacon->getVehicleId() == vehicleData.leaderId) {
					traci->commandSetControllerFakeData(traci->getExternalId(), 0, -1, 0, beacon->getSpeed(), beacon->getAcceleration());
				}
				if (beacon->getVehicleId() == vehicleData.frontId) {
					...
					//get my position
					Coord position = traci->getCurrentPosition();
					//compute distance (-4 because of vehicle length)
					double distance = position.distance(frontPosition) - 4;
					traci->commandSetControllerFakeData(traci->getExternalId(), distance, beacon->getSpeed(), beacon->getAcceleration(), -1, 0);
